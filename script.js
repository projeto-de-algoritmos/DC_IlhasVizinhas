...
let conquer = new Audio("assets/conquer.mp3");
conquer.volume = 0.5;
...
function limit_input_ilhas(input) {
  if (input.value > 100) {
    input.value = input.max;
  }
}
...
function criarGrid2() {
  ...
  grid2.style.grid2Templatecolunas = `repeat(${10}, 1fr)`;
...
function ilhasSquare2() {
  this.classList.add("hover");
}

function tirarIlhasSquare2() {
  this.classList.remove("hover");
}

function randomizarIlhas(count) {
  const squares2 = document.getElementsByClassName("square2");
  const totalSquares2 = squares2.length;
  const indices = Array.from({ length: totalSquares2 }, (_, i) => i);
  
  for (let i = 0; i < squares2.length; i++) {
    squares2[i].classList.remove("closest");
  }

  for (let i = 0; i < count; i++) {
    const randomIndex = Math.floor(Math.random() * indices.length);
    const ilhasIndex = indices[randomIndex];
    const square2 = squares2[ilhasIndex];
    square2.classList.add("ilhas");
    indices.splice(randomIndex, 1);
  }
}

function limparIlhas() {
  const squares2 = document.getElementsByClassName("square2");
  for (let i = 0; i < squares2.length; i++) {
    squares2[i].classList.remove("ilhas");
  }
}
...
function dividirEConquistar() {
  ...
  const squares2 = document.getElementsByClassName("square2");
  const ilhasSquares2 = document.getElementsByClassName("ilhas");

  const pontosProximos = acharPontosProximos(Array.from(ilhasSquares2));

  for (let i = 0; i < squares2.length; i++) {
    squares2[i].classList.remove("closest");
  }

  for (let i = 0; i < pontosProximos.length; i++) {
    pontosProximos[i][0].classList.add("closest");
    pontosProximos[i][1].classList.add("closest");
  }
}

function acharPontosProximos(points) {
  if (points.length < 2) {
    return [];
  }
  ...
  for (let i = 0; i < points.length; i++) {
    for (let j = i + 1; j < points.length; j++) {
      ...
      if (distance < minDistance) {
        ...
        pontosProximos.push([points[i], points[j]]);
...
document.getElementById("randomizar").addEventListener("click", function () {
  restart.play();
  const ilhasCount = parseInt(document.getElementById("ilhas").value);
  limparIlhas();
  randomizarIlhas(ilhasCount);
});

document.getElementById("dividirEConquistar").addEventListener("click", dividirEConquistar);
